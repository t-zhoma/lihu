...
CanvasRenderer.prototype.drawLeftBox = function() {
    ...
    for (i = 0; i < game.lb.cardsNum; i++) {
        this.ctx.drawImage(img, x, y, Game.CARD_HEIGHT, Game.CARD_WIDTH);
        y += Game.CARD_SPACE;
    }
}
...
CanvasRenderer.prototype.drawTopBox = function() {
    ...
    for (i = 0; i < game.tb.cardsNum; i++) {
        this.ctx.drawImage(img, x, y, Game.CARD_WIDTH, Game.CARD_HEIGHT);
        x += Game.CARD_SPACE;
    }
}
...
CanvasRenderer.prototype.drawBottomBox = function() {
    ...
    for (i = 0; i < game.bb.cards.length; i++) {
        ...
        this.ctx.drawImage( game.bb.cards[i].picture, x, realY, Game.CARD_WIDTH, Game.CARD_HEIGHT);
        game.bb.cards[i].x = x;
...
CanvasRenderer.prototype.drawBottomOutbox = function(cards) {
    ...
    for (i = 0; i < cards.length; i++) {
        this.ctx.drawImage(cards[i].picture, x, y, Game.CARD_WIDTH, Game.CARD_HEIGHT);
        x += Game.CARD_SPACE;
    }
}
...
var Source = {
    ...
    getImg: function(src) {
        ...
        if ( Source.images == false  ) return false;
        // todo 
        // add reload or handle
        return Source.images[src];
    }
};
